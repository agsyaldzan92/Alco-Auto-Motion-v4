  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-center justify-center p-3 sm:p-6 overflow-y-auto">
      <div className="alco-modal max-w-4xl w-full p-5 sm:p-6 shadow-2xl space-y-5 relative animate-fade-in my-auto">
        ...
        {/* 5. Stage 4: Final Output & Exports */}
        <FinalOutputStage
          renderedBlobUrl={renderedBlobUrl}
          finalReadiness={finalReadiness}
          exportedFormat={exportedFormat}
          selectedTier={selectedTier}
          audioMuxPending={audioMuxPending}
          isDownloading={isDownloading}
          downloadSuccess={downloadSuccess}
          downloadError={downloadError}
          onVerifiedDownload={handleVerifiedDownload}
          isRendering={isRendering}
          onStartRenderCurrentTier={handleStartRenderCurrentTier}
          onSelectTier={setSelectedTier}
          onStartServerMp4Render={handleStartServerMp4Render}
          onStartWebmRender={(tier) => handleStartWebmRender(tier === 'standard_24fps' ? 'standard_24fps' : 'safe_20fps')}
          backendMode={backendMode}
          hasFailCheck={hasFailCheck}
          onDownloadFFmpegPackage={handleDownloadFFmpegPackage}
          onDownloadSrt={handleDownloadSrt}
          onDownloadJson={handleDownloadJson}
        />

        {/* 6. Advanced Diagnostics & Quick Fix Accordion */}
        <AdvancedDiagnosticsAccordion
          diagnosticInfo={diagnosticInfo}
          ...
          backendMode={backendMode}
          backendStatusReason={backendStatusReason}
          healthEndpointDiag={healthEndpointDiag}
          pingEndpointDiag={pingEndpointDiag}
          auditResult={auditResult}
          ...
          setHookReviewState={setHookReviewState}
          captionReviewState={captionReviewState}
          setCaptionReviewState={setCaptionReviewState}
          sfxReviewState={sfxReviewState}
          setSfxReviewState={setSfxReviewState}
          brollReviewState={brollReviewState}
          setBrollReviewState={setBrollReviewState}
          talkingHeadReviewState={talkingHeadReviewState}
          setTalkingHeadReviewState={setTalkingHeadReviewState}
          quickFixToast={quickFixToast}
          handleFixHook={handleFixHook}
          handleShortenCaption={handleShortenCaption}
          handleMoveCaptionUp={handleMoveCaptionUp}
          handleReduceSfx={handleReduceSfx}
          handleDisableBroll={handleDisableBroll}
          handleEnhanceTalkingHead={handleEnhanceTalkingHead}
          handleCopyDiagnosticReport={handleCopyDiagnosticReport}
          copiedDiagnostic={copiedDiagnostic}
          showDiagnosticDetails={showDiagnosticDetails}
          setShowDiagnosticDetails={setShowDiagnosticDetails}
          buildRenderDiagnosticReport={buildRenderDiagnosticReport}
        />
      </div>
    </div>
  );
